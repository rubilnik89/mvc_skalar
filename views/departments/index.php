...
<body>
	<form method="post" action="/emploees/<?php echo $department.'/'.($page+1).'/'.$limit ?>">
    ...
    <div class="container" role="main">
        ...
        <?php //PAGINATION
        if ($page>=1) {
            echo '<a href="/emploees/'.$department.'/1/' . $limit .'"><<</a> &nbsp; ';
            echo '<a href="/emploees/'.$department.'/' . $page .'/' . $limit .
                '">< </a> &nbsp; ';
        }
        for ($j = 1; $j<$count_pages; $j++) {
            if ($j>=$start && $j<=$end) {
                if ($j==($page+1)) echo '<a href="/emploees/'.$department.'/' . $j . '/'.$limit.'"><strong style="color: #df0000">' . $j .
                    '</strong></a> &nbsp; ';
                else echo '<a href="/emploees/'.$department.'/' .
                    $j . '/'.$limit.'">' . $j . '</a> &nbsp; ';
            }
        }
        if ($j>$page && ($page+2)<$j) {
            echo '<a href="/emploees/'.$department.'/' . ($page+2) .'/'.$limit.
                '"> ></a> &nbsp; ';
            echo '<a href="/emploees/'.$department.'/' . ($j-1) .'/'.$limit.
                '">>></a> &nbsp; ';
        }
        ?>
    </div>
